...
def tasks_tab():
    ...
    if role == "control":
        ...
        st.dataframe(df, use_container_width=True)
    # For editing/adding tasks for current side
    # ...existing code...
    add_key = f"add_{side if side else role}"
    task_no = st.text_input("Task No", key=f"add_task_no_{add_key}")
    desc = st.text_input("Description", key=f"add_desc_{add_key}")
    force_group = st.text_input("Force Group", key=f"add_fg_{add_key}")
    type_ = st.selectbox("Type", ["T", "I"], key=f"add_type_{add_key}")
    criteria = st.text_input("Criteria", key=f"add_criteria_{add_key}")
    stated = st.slider("Stated %", 0, 100, 0, key=f"add_stated_{add_key}")
    achieved = st.slider("Achieved %", 0, 100, 0, key=f"add_achieved_{add_key}") if role == "control" else None
    intangible = st.selectbox("Intangible", ["nil", "partial", "complete"], key=f"add_intangible_{add_key}") if role == "control" else None
    # Get all DP numbers and objectives for the current force
    dp_options = [dp.get("DP No") or dp.get("dp_no") for dp in data.get("dps", []) if dp.get("DP No") or dp.get("dp_no")]
    obj_options = [obj.get("Name") or obj.get("name") for obj in data.get("objectives", []) if obj.get("Name") or obj.get("name")]
    dp_no = st.selectbox("DP No", dp_options, key=f"add_dpno_{add_key}")
    objectives = st.multiselect("Objectives", obj_options, key=f"add_obj_{add_key}")
    if st.button("Add Task", key=f"add_task_btn_{add_key}") and task_no and desc and dp_no:
        task = {
            "Task No": task_no,
            "Desc": desc,
    ...
    if role in ["control"] + SIDES:
        ...
        if st.button("Add Task", key="add_task_btn") and task_no and desc:
            if role == "control":
                ...
                data = load_project(project, role)
                task = {"Task No": task_no, "Desc": desc, "Force Group": force_group, "Type": type_, "Criteria": criteria, "Stated %": stated, "DP No": dp_no}
                if achieved is not None:
                    task["Achieved %"] = achieved
                if intangible is not None:
                    task["Intangible"] = intangible
                data["tasks"].append(task)
                save_project(project, role, data)
                st.success(f"Task '{desc}' added.")
            st.rerun()
        if st.session_state.get("role") in ["control", side]:
            st.markdown("### Edit Tasks")
            if tasks:
                task_labels = [f"Task {task.get('Task No', i+1)}: {task.get('Desc','')}" for i, task in enumerate(tasks)]
                selected_idx = st.selectbox("Select Task to Edit", list(range(len(tasks))), format_func=lambda x: task_labels[x], key=f"edit_task_select_{side}")
                task = tasks[selected_idx]
                unique_key = f"{side}_{task.get('DP No','none')}_{selected_idx}"
                task_no = st.text_input("Task No", task.get("Task No", ""), key=f"edit_task_no_{unique_key}")
                desc = st.text_input("Description", task.get("Desc", ""), key=f"edit_desc_{unique_key}")
                force_group = st.text_input("Force Group", task.get("Force Group", ""), key=f"edit_fg_{unique_key}")
                type_ = st.selectbox("Type", ["T", "I"], index=["T", "I"].index(task.get("Type", "T")), key=f"edit_type_{unique_key}")
                criteria = st.text_input("Criteria", task.get("Criteria", ""), key=f"edit_criteria_{unique_key}")
                stated = st.slider("Stated %", 0, 100, int(task.get("Stated %", 0)), key=f"edit_stated_{unique_key}")
                achieved = st.slider("Achieved %", 0, 100, int(task.get("Achieved %", 0)), key=f"edit_achieved_{unique_key}") if st.session_state.get("role") == "control" else None
                intangible = st.selectbox("Intangible", ["nil", "partial", "complete"], index=["nil", "partial", "complete"].index(task.get("Intangible", "nil")), key=f"edit_intangible_{unique_key}") if st.session_state.get("role") == "control" else None
                dp_no = st.selectbox("DP No", dps, index=dps.index(task.get("DP No", dps[0])) if task.get("DP No") in dps else 0, key=f"edit_dpno_{unique_key}")
                if st.button("Save Task", key=f"save_task_{unique_key}"):
            ...
            dps = [dp.get("DP No") for dp in data.get("dps", []) if dp.get("DP No")]
...
def progress_entry_tab():
    ...
    for side in SIDES:
        ...
        dps = data.get("dps", [])
        color = FORCE_COLORS.get(side, "#0f172a")
        st.markdown(f"<h3 style='color:{color}'>{side.capitalize()} Force</h3>", unsafe_allow_html=True)
        if not dps:
            st.info(f"No DPs found for {side}. Please add DPs in the Decisive Points tab.")
            continue
        dp_titles = {dp.get("DP No"): dp.get("Name", dp.get("DP No")) for dp in dps}
        # Group tasks by DP
        dp_map = {dp.get("DP No"): [] for dp in dps if dp.get("DP No")}
        for i, task in enumerate(tasks):
            dp_no = task.get("DP No")
            task_no = task.get("Task No")
            if not dp_no or not task_no:
                continue
            if dp_no in dp_map:
                dp_map[dp_no].append((i, task))
        for dp_no in dp_map:
            dp_title = dp_titles.get(dp_no, dp_no)
            st.markdown(f"<div style='margin-left:16px;font-weight:bold'>DP {dp_no}: {dp_title}</div>", unsafe_allow_html=True)
            if not dp_map[dp_no]:
                st.warning(f"No tasks found for DP {dp_no}.")
            for i, task in dp_map[dp_no]:
                st.markdown(f"<b>Task {task.get('Task No')}</b>: {task.get('Desc')}", unsafe_allow_html=True)
                st.write(f"Force Group: {task.get('Force Group', '')}")
                st.write(f"Type: {task.get('Type', '')}")
                st.write(f"Criteria: {task.get('Criteria', '')}")
                st.write(f"Stated %: {task.get('Stated %', '')}")
                st.write(f"Achieved %: {task.get('Achieved %', '')}")
                st.write(f"Intangible: {task.get('Intangible', '')}")
                weight = st.slider(f"Weight % ({side} DP {dp_no} Task {i+1})", 0, 100, int(task.get("Weight %", 100)), key=f"wgt_{side}_{dp_no}_{i}")
                achieved = st.slider(f"Achieved % ({side} DP {dp_no} Task {i+1})", 0, 100, int(task.get("Achieved %", 0)), key=f"ach_{side}_{dp_no}_{i}")
                if st.button(f"Save {side} DP {dp_no} Task {i+1}", key=f"save_{side}_{dp_no}_{i}"):
                    task["Weight %"] = weight
                    task["Achieved %"] = achieved
                    tasks[i] = task
                    data["tasks"] = tasks
